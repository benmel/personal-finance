...
#first, get means and vars for the individual columns that match
    ...
    $sql = "select count(*),avg(l.$field1),stddev(l.$field1),avg(r.$field2),stddev(r.$field2) from all_data l join all_data r on l.timestamp= r.timestamp where l.symbol='$s1' and r.symbol='$s2'";
    ...
    if ($count<30) { # not enough data
      ...
      $sql = "select avg((l.$field1 - $mean_f1)*(r.$field2 - $mean_f2)) from all_data l join all_data r on  l.timestamp=r.timestamp where l.symbol='$s1' and r.symbol='$s2'";
      $sql.= " and l.timestamp>= $from" if $from;
      $sql.= " and l.timestamp<= $to" if $to;
